...
Sistemul va conține o clasă `Sistem` care va conține o listă de utilizatori și metode pentru a adăuga utilizatori noi, a afișa utilizatorii existenți 
și a verifica nivelul de acces al unui utilizator.
La sistem va avea acces doar Utilizatorii Admin așa că trebuie să ne asigurăm că aceștia vor avea metode pentru a adăuga, a modificare și a șterge 
datele private ale sistemului.
...
2. Creează o clasă `Sistem` care va conține un atribut privat `__utilizatori` inițializat cu un dicționar gol în care cheile vor fi id-ul și valorile utilizatorii.
    Clasa `Sistem` trebuie să conțină metoda `adauga_utilizator` care va primi un obiect de tip `Utilizator` și va adăuga utilizatorul la dicționar împreună 
    cu un nou id.
    ...
    Clasa `Sistem` trebuie să conțină și metoda `modifica_name_user` care va primi id-ul utilizatorului și noul nume al utilizatorului și va modifica numele 
    ...
    Clasa `Sistem` trebuie să conțină și metoda `modifica_nivel_acces` care va primi id-ul utilizatorului și noul nivel de acces al utilizatorului și va modifica 
    nivelul de acces al utilizatorului respectiv.
"""

# CODUL TĂU VINE MAI JOS:
class user:
    def __init__(self,_nume):
        self._nume = _nume
        self.__nivel_acces__ = "Default"
    @property
    def nume(self):
        return self._nume
    @nume.setter
    ...
    def nivel_acces(self):
        return self.__nivel_acces__ 
    @nivel_acces.setter
    def nivel_acces(self,valoare):
        self.__nivel_acces__ = valoare
...
class Sistem:
    ...
    def adauga_utilizator(self,utilizator):
        self.id+=1
        self.__utilizatori[self.id] = utilizator
    ...
    def modifica_name_user(self,id_cautat,nume_nou):
        for id,utilizator in self.__utilizatori.items():
            if id_cautat == id:
                utilizator.nume = nume_nou
    def sterge_utilizator(self,id_cautat):
        for id in self.__utilizatori.keys():
            if id_cautat == id:
                del self.__utilizatori[id]
            break
    def modifica_nivel_acces(self,id_cautat, acces_nou):
        for id,utilizator in self.__utilizatori.items():
            if id_cautat == id:
                utilizator.nivel_acces = acces_nou
# CODUL TĂU VINE MAI SUS:
...
class TechSolutionsApp:
    ...
    @classmethod
    def delogat_view(self):
        return f"Versiunea aplicației este {self.versiune_aplicatie}"
