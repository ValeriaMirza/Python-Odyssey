...
# CODUL TĂU VINE MAI JOS:
def task_2(*args):
    lista = [i for i in args if isinstance(i,int)]
    return lista
# CODUL TĂU VINE MAI SUS:
...
def task_5(*args, **kwargs):
    ...
    list_of_kwargs = [key for key,value in kwargs.items() if isinstance(value,str)]
    list_of_args.sort()
    list_of_kwargs.sort()
...
"""
Task: Creați o funcție cu numele `task_7` care poate primi un număr nedeterminat de argumente atât string-uri cât și numere 
și va returna un dicționar cu două chei: `str` și `int`. Cheia `str` va avea o listă cu toate string-urile primite ca argumente,
...
# CODUL TĂU VINE MAI JOS:
def task_7(*args):
    strings = [i for i in args if isinstance(i,str)]
    numbers = [i for i in args if isinstance(i,int)]
    return {'str': strings, 'int': numbers}
# CODUL TĂU VINE MAI SUS:
...
"""
Task: Creați o funcție cu numele `task_8` care primește un număr variabil de argumente și returnează un dicționar cu 
două chei: `palindrom` și `non_palindrom`.  Cheia `palindrom` va avea o listă cu toate argumentele care sunt palindroame, 
...
# CODUL TĂU VINE MAI JOS:
def task_8(*args):
    palindrom = [i for i in args if i == i[::-1]]
    non_palindrom = [i for i in args if i != i[::-1]]
...
"""
Task: Creați o funcție cu numele `task_9` care primește un număr variabil de argumente de tip integer și un argument `number` 
de tip integer.
...
def task_9(*args,number):
    multipli=[i for i in args if(i%number==0)]
    return multipli
# CODUL TĂU VINE MAI SUS:
...
def task_10(*args,number):
    divizori = [i for i in args if (number % i == 0)]
    return divizori
# CODUL TĂU VINE MAI SUS:
...
# CODUL TĂU VINE MAI JOS:
def task_11(*args):
    if len(args) < 2 or args[0] != 1 or args[1] != 1:
        return False
    
    for a,b,c in zip(args,args[1:], args[2:]):
        if(a+b!=c):
            return False
...
"""
Task: Creați o funcție cu numele `task_13` care primește obligatoriu un argument de tip string și un număr variabil de 
argumente de tip string. Funcția va returna True dacă toate argumentele sunt anagrame și False în caz contrar.
...
"""
Task: Creați o funcție cu numele `task_14` care primește un argument `sub_string` de tip string și un 
...
def task_14(*args,sub_string):
    lista = [i for i in args if sub_string in i ]
    return lista
# CODUL TĂU VINE MAI SUS:
...
"""
Task: Creați o funcție cu numele `task_15` care primește un argument `sub_string` de tip string și un număr variabil de 
argumente de tip string. Funcția va returna un dicționar cu două chei: `contains` și `not_contains`.
Cheia `contains` va avea o listă cu toate argumentele care conțin `sub_string`, iar cheia `not_contains` 
va avea o listă cu toate argumentele care nu conțin `sub_string`.
Exemplu: task_15('home', 'same', 'meme', sub_string = 'me') ➞ {'contains': ['home', 'same', 'meme'], 'not_contains': []}
"""

# CODUL TĂU VINE MAI JOS:
def task_15(*args,sub_string):
    contains = [i for i in args if sub_string in i]
    not_contains = [i for i in args if sub_string not in i]
...
"""
Task: Creați o funcție cu numele `task_16` care va primi un număr variabil de argumente de tip integer și un 
argument `ooperation` de tip string.
...
"""
Task: Creați o funcție cu numele `task_18` care primește un număr variabil de argumente de tip string și va returna 
un dicționar în care cheile vor fi caracterele întâlnite în argumentele primite, iar valorile vor fi numărul de apariții ale caracterelor.
...
"""
Task: Creați o funcție cu numele `task_19` care primește un număr variabil de argumente de tip integer și va 
returna un dicționar în care cheile vor fi numerele prime întâlnite în argumentele primite, iar valorile vor fi 
numărul de apariții ale numerelor prime.
...
"""
Task: Creați o funcție cu numele `task_20` care primește un număr variabil de argumente de 
tip string și va returna un dicționar în care cheile vor fi lungimile cuvintelor întâlnite în argumentele primite,
